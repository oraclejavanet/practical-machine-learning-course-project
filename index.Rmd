...
fitControlRpart <- rpart.control(method = "cv", number = 3, verboseIter = TRUE)
fitControlTrain <- trainControl(method = "cv", number = 3, verboseIter = TRUE)
```
...
```{r decision-tree-model-fit, message = FALSE, echo = TRUE}
set.seed(660067)
modFitDecisionTree <- rpart(as.factor(classe) ~ .,
                            data = trainSet,
                            control = fitControlRpart,
                            method = "class")
prp(modFitDecisionTree,
    faclen = 0,
    box.palette = "GnBu",
    cex = 0.50,
    legend.x = 0,
...
well it performed by reviewing the "accuracy" variable.

```{r decision-tree-prediction, echo = TRUE}
predictionDecisionTree <- predict(modFitDecisionTree, newdata = testSet, type = "class")
confMatrixDecisionTree <- confusionMatrix(predictionDecisionTree, testSet$classe)
print(confMatrixDecisionTree)
```

Plot the matrix results.

```{r decision-tree-matrix-results, echo = TRUE}
g <- ggplot(data = as.data.frame(confMatrixDecisionTree$table),
...
g <- g + ggtitle(paste0("Decision Tree Accuracy ",
                        round(confMatrixDecisionTree$overall['Accuracy'], 3) * 100,
                        "%"))
print(g)
```

The results from the confusion matrix for the Decision Tree model show a
predicted accuracy of
`r round(confMatrixDecisionTree$overall['Accuracy'], 3)` giving an out-of-sample
error rate of `r 1 - round(confMatrixDecisionTree$overall['Accuracy'], 3)` which
is considerable.
